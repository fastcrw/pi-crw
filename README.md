...
```bash
npx github:fastcrw/pi-crw install      # or: bunx github:fastcrw/pi-crw install
```
...
```bash
npx github:fastcrw/pi-crw status       # is it installed? which backend?
npx github:fastcrw/pi-crw uninstall    # remove it
```

> Installed straight from the GitHub repo (no npm registry). For a pinned
> version use `npx github:fastcrw/pi-crw#v0.1.0 install`, or clone and run
